...
public:
     ...
     */
    VirtualMachine* AttachState(int api, lua_State* L);
    
    void VMInitialize(int api, lua_State* L, VirtualMachine* vm);

    /**
     * Detaches the debugger from the state.
     */
    void DetachState(int api, lua_State* L);

    /**
     * Sends information about the script to the front end. This is called after
     * calling the lua_load (or similar) method.
     */
    int PostLoadScript(int api, int result, lua_State* L, const char* source, size_t size, const char* name);
    ...
    int RegisterScript(int api, lua_State* L, lua_Debug* ar);
     ...
     */
    bool Evaluate(int api, lua_State* L, const std::string& expression, int stackLevel, std::string& result);
     ...
     */
    TiXmlNode* GetValue(int api, lua_State* L, const std::string& variable, unsigned int scriptIndex,
        unsigned int line);
     ...
     */
    int Call(int api, lua_State* L, int nargs, int nresults, int errorfunc);
    ...
    bool StackHasBreakpoint(int api, lua_State* L);
     ...
     */
    bool GetClassNameForMetatable(int api, lua_State* L, int mt) const;

    /**
     * Pushes onto the stack the class name for a userdata.
     */
    const char* GetClassNameForUserdata(int api, lua_State* L, int ud) const;
     ...
     */
    void RegisterClassName(int api, lua_State* L, const char* name, int metaTable);
     ...
     * occurs.
     */
    void HookCallback(int api, lua_State* L, lua_Debug* ar);

    /**
     * Called when a new API is created.
     */
    void CreateApi(int api);
     ...
     */
    bool EnableJit(int api, lua_State* L, bool enable);
...
private:
     ...
     * is resumed.
     */
    void BreakFromScript(int api, lua_State* L);

    /**
     * Error handling call back for relaying exceptions.
     */
    int ErrorHandler(int api, lua_State* L);
     ...
     * handler we defined.
     */
    void SendBreakEvent(int api, lua_State* L, int stackTop = 0);
     ...
     */
    TiXmlNode* GetValueAsText(int api, lua_State* L, int n, int maxDepth = 10, const char* typeNameOverride = NULL, bool displayAsKey = false) const;

    /**
     * Gets the value at location n on the stack as text. If expandTable is true
     * then tables will be returned in their expanded form (i.e. "{ ... }")
     */
    TiXmlNode* GetLuaBindClassValue(int api, lua_State* L, unsigned int maxDepth, bool displayAsKey = false) const;

    /**
     * Gets the table value at location n on the stack as text. Nested tables are
     * not expanded.
     */
    TiXmlNode* GetTableAsText(int api, lua_State* L, int t, int maxDepth = 10, const char* typeNameOverride = NULL) const;
     ...
     */
    int LoadScriptWithoutIntercept(int api, lua_State* L, const char* string, size_t size, const char* name);

    /**
     * This is semantically equivalent to luaL_loadstring.
     */
    int LoadScriptWithoutIntercept(int api, lua_State* L, const std::string& string);
...
private:
     ...
     */
    bool CreateEnvironment(int api, lua_State* L, int stackLevel, int nilSentinel);
     ...
     */
    void ChainTables(int api, lua_State* L, int child, int parent);

    /**
     *
     */
    void CreateChainedTable(int api, lua_State* L, int nilSentinel, int localTable, int upValueTable, int globalTable);
     ...
     * to nils.
     */
    void SetLocals(int api, lua_State* L, int stackLevel, int localTable, int nilSentinel);
     ...
     * to nils.
     */
    void SetUpValues(int api, lua_State* L, int stackLevel, int upValueTable, int nilSentinel);

    /**
     * Sets the function (on the top of the Lua stack) to be called when a Lua
     * object is garbage collected.
     */
    void SetGarbageCollectionCallback(int api, lua_State* L, int index);

    /**
     * Creates a new table with weak keys or values (specified by setting the type as "k" or "v").
     */
    void CreateWeakTable(int api, lua_State* L, const char* type);
     ...
     */
    void CreateGarbageCollectionSentinel(int api, lua_State* L);
    ...
    /**
     */
    static int IndexChained(int api, lua_State* L);

    /**
     */
    static int NewIndexChained(int api, lua_State* L);
     ...
     */
    void LogHookEvent(int api, lua_State* L, lua_Debug* ar);

    void UpdateHookMode(int api, lua_State* L, lua_Debug* hookEvent);
     ...
     */
    bool CallMetaMethod(int api, lua_State* L, int valueIndex, const char* method, int numResults, int& result) const;
     ...
     */
    void MergeTables(int api, lua_State* L, unsigned int tableIndex1, unsigned int tableIndex2) const;

    /**
     * Gets the number of functions on the Lua stack.
     */
    int GetStackDepth(int api, lua_State* L) const;
     ...
     * call stack.
     */
    unsigned int GetUnifiedStack(int api, const StackEntry nativeStack[], unsigned int nativeStackSize,
